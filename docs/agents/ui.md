...
- Tall Page Chips and Domain Card outlines use the approved CSS `superellipse(1.7)` midpoint (ADRs 0055 and 0059); compact Page Chips keep the measured capsule. All four corners share one fixed radius per surface: 20.5px for tall chips and 38.5px for cards, independent of card contents. Preserve natural height, CSS radius overlap reduction, and existing spacing. Other non-capsule rectangles retain their existing shapes; do not introduce asymmetric or content-dependent card corners.
...
- The expansion overlay carries the same context qualifier and is an `aria-hidden` copy that owns pointer interaction while the canonical chip retains keyboard and accessibility identity. Give it only the part anchor; keep the single Page Chip landmark on the canonical surface throughout expansion.
