...
- Non-capsule rectangles use `corner-shape: squircle` with the same radius at all four corners. Page Chip squircles align with the one-line capsule by boundary overlap; Domain Card outlines use their own larger fixed radius token, independent of chip contents; preserve natural card height and allow CSS overlap reduction. Preserve this contract instead of introducing custom non-capsule contours, asymmetric corners, or content-dependent card radii.
