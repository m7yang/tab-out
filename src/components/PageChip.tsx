...
function usePageChipElement({ chip, filter = '', layoutScope = '', groupContinuesAbove = false, groupContinuesBelow = false, suppressedTitleToneByText }: PageChipProps) {
  ...
  function envLabelNode(env: DashboardChipEnv, mode: ChipTextRenderMode) {
    ...
    const envDimmed = envClosed || (!isReadOnlyDashboardSourceType(envSourceType) && !!env.suspended)
    ...
    const envLabelContent = (
      <span className={cn('chip-env-label', envDimmed && VARIANT_LABEL_DIM_CLASS_NAME)}>
        {highlightedTextNodes(env.prefix, highlightTerms, `${mode}-env-${env.prefix}`)}
      </span>
    )
  ...
  // `.page-chip`: the chip has rounded corners
  ...
  const hoverMatchOutline = hoverMatched && !chip.iconOnly ? (
    <span
      ...
      className="page-chip-hover-match-outline pointer-events-none absolute inset-0 z-3 rounded-page-chip outline-1 outline-offset-1 outline-(--accent-amber) [corner-shape:superellipse(1.7)] [--capsule-fill:transparent]"
      aria-hidden="true"
    />
  ) : null
