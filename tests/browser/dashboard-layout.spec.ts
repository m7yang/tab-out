...
for (const { kind, selector, titles, paintTitle } of [
  ...
  { kind: 'colored suppression', selector: '.chip-title-suppression-marker', titles: ['Tooltip Boundary Alpha'], paintTitle: 'Tooltip Boundary Alpha' },
]) {
  ...
  for (const truncated of kind === 'structural' ? [false, true] : [false]) {
    for (const fraction of [0.25, 0.5, 0.75]) {
      test(`${kind} marker keeps its painted left edge at fraction ${fraction} (truncated: ${truncated})`, async ({ browser, baseURL }) => {
        ...
        try {
          if (truncated) {
            await page.route('**/tests/fixtures/dashboard-resize.html', async (route) => {
              ...
              const body = (await response.text()).replace(
                'example-owner/example-long-repository at story/ABC-123_2',
                'example-owner/example-long-repository: A speedy tool for processing example content with useful results and documentation',
              )
          ...
          const chip = page.locator('[data-tabout="page-chip"][data-tabout-context="domain-card"]').filter({ hasText: truncated ? 'A speedy tool' : paintTitle }).first()
          await chip.scrollIntoViewIfNeeded()
          await page.mouse.move(0, 0)
          const marker = chip.locator(selector).first()
          await expect(marker).toHaveAttribute('data-capsule-ready', '')
          if (truncated) await expect(chip.locator('.chip-text')).toHaveClass(/chip-text-truncated/u)
          await marker.evaluate((element, fraction) => {
            const text = element.closest<HTMLElement>('.chip-text')
            if (!text) throw new Error('Marker text container is unavailable')
            const left = element.getBoundingClientRect().left
            text.style.marginLeft = `${Math.floor(left) + fraction - left}px`
          }, fraction)
          // Title measurement can replace the marker while the new margin settles.
          let resting = await marker.boundingBox()
          await expect.poll(async () => {
            resting = await marker.boundingBox()
            return resting && Math.abs(resting.x - Math.floor(resting.x) - fraction) < 0.01
          }).toBe(true)
