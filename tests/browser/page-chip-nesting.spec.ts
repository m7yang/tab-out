...
test('a one-line card keeps its compact height when the fixed outline appears', async ({ page }) => {
  ...
  await expect(page.locator(cardFrame)).toHaveCSS('corner-shape', 'superellipse(1.7)')
...
test('card corners stay fixed after chip reflow and while using local frames', async ({ page }) => {
  ...
  await expect(page.locator(cardFrame)).toBeVisible()
  await expect(page.locator(cardFrame)).toHaveCSS('border-radius', '38.5px')
  await expect.poll(() => card.evaluate((element) => getComputedStyle(element, '::after').borderRadius)).toBe('38.5px')
  await expect.poll(() => card.evaluate((element) => getComputedStyle(element, '::after').getPropertyValue('corner-shape'))).toBe('superellipse(1.7)')
  await expect(page.locator(cardFrame)).toHaveCSS('corner-shape', 'superellipse(1.7)')
