...
test('PageChip renders same-title URL variants below one visible title', () => {
  ...
  assert.match(requiredAt(titleVariantListMatch, 1), /(?:^|\s)pr-\[2\.5px\](?:\s|$)/)
  assert.match(requiredAt(titleVariantListMatch, 1), /(?:^|\s)pb-\[1\.5px\](?:\s|$)/)
  ...
  assert.match(requiredAt(titleVariantButtonMatch, 1), /rounded-\[7px\]/)
  ...
  assert.match(pageChipSource, /row\.current && 'bg-neutral-600\/10 text-tab-live'/)
...
test('PageChip expands same-title URL variant groups in place', () => {
  ...
  assert.match(layoutFragmentsSource, /chip-title-variant-list inline-flex max-w-full flex-col items-stretch pr-\[2\.5px\] pb-\[1\.5px\] align-top'/)
  assert.match(layoutFragmentsSource, /chip-title-variant inline-flex max-w-full min-w-0 items-center gap-1 rounded-\[7px\] bg-transparent px-1\.5 py-\[3px\]/)
...
test('TabHistoryPanel uses PageChip-style fade truncation and in-place title expansion', () => {
  ...
  assert.match(historyEntryComponentSource, /HISTORY_ENTRY_INTERACTION_CLASSES = 'title-interaction:hover:bg-\(--history-entry-interaction-bg\)[\s\S]*\[\&\.history-entry-expanded-open\]:bg-\(--history-entry-interaction-bg\)/)
  assert.match(historyEntryComponentSource, /HISTORY_ENTRY_ACTIVE_OTHER_INTERACTION_CLASSES = `bg-\(--history-entry-rest-bg\)/)
