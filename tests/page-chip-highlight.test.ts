...
test('PageChip renders same-title URL variants below one visible title', () => {
  ...
  assert.match(requiredAt(chipMatch, 1), /(?:^|\s)pr-\[2\.5px\](?:\s|$)/)
  assert.match(requiredAt(chipMatch, 1), /(?:^|\s)pb-\[2\.5px\](?:\s|$)/)
  ...
  assert.match(requiredAt(titleVariantActionOwnerMatch, 1), /\bsize-5\b/)
  assert.match(requiredAt(titleVariantActionOwnerMatch, 1), /\bcursor-pointer\b/)
  assert.match(requiredAt(titleVariantActionMatch, 1), /\bsize-5\b/)
...
test('PageChip expands same-title URL variant groups in place', () => {
  ...
  assert.match(layoutFragmentsSource, /chip-title-variant-list inline-flex max-w-full flex-col items-stretch align-top'/)
  assert.match(layoutFragmentsSource, /chip-title-variant inline-flex max-w-full min-w-0 items-center gap-1 rounded-full bg-transparent px-1\.5 py-\[3px\]/)
