...
test('PageChip highlights matched filter keywords inside visible chip text', () => {
  ...
  assert.match(requiredAt(chipMatch, 1), /\bcursor-pointer\b/)
  assert.doesNotMatch(requiredAt(chipMatch, 1), /\bcursor-default\b/)
})
...
test('PageChip renders closed saved pages muted with grouped hover treatment and no close action', () => {
  ...
  assert.doesNotMatch(requiredAt(chipMatch, 1), /\[--capsule-fill:var\(--chip-rest-bg\)\]/)
  assert.match(requiredAt(chipMatch, 1), /\bhover:not-focus-visible:not-data-\[tabout-filter-result-selected=true\]:outline\b/)
  assert.match(requiredAt(chipMatch, 1), /hover:not-focus-visible:not-data-\[tabout-filter-result-selected=true\]:outline-\(--chip-hover-border\)/)
...
test('PageChip renders same-title URL variants below one visible title', () => {
  ...
  assert.match(requiredAt(chipMatch, 1), /hover:\[--capsule-fill:var\(--chip-interaction-bg\)\]/)
  assert.match(requiredAt(chipMatch, 1), /hover:not-focus-visible:not-data-\[tabout-filter-result-selected=true\]:outline-1/)
  assert.match(requiredAt(chipMatch, 1), /hover:not-focus-visible:not-data-\[tabout-filter-result-selected=true\]:-outline-offset-1/)
  assert.match(requiredAt(chipMatch, 1), /hover:not-focus-visible:not-data-\[tabout-filter-result-selected=true\]:outline-\(--chip-hover-border\)/)
  ...
  assert.doesNotMatch(requiredAt(titleVariantListMatch, 1), /\bdivide-y\b/)
  assert.doesNotMatch(requiredAt(titleVariantListMatch, 1), /\bdivide-neutral-500\/15\b/)
  assert.match(requiredAt(chipMatch, 1), /(?:^|\s)pr-\[4\.5px\](?:\s|$)/)
  assert.match(requiredAt(chipMatch, 1), /(?:^|\s)pb-\[4\.5px\](?:\s|$)/)
  ...
  assert.match(requiredAt(titleVariantActionsMatch, 1), /-left-6\.5/)
  ...
  assert.match(requiredAt(titleVariantActionOwnerMatch, 1), /\bh-4\.5 w-5\b/)
  assert.match(requiredAt(titleVariantActionOwnerMatch, 1), /\bcursor-pointer\b/)
  assert.match(requiredAt(titleVariantActionMatch, 1), /\bh-4\.5 w-5\b/)
  ...
  assert.match(requiredAt(titleVariantButtonMatch, 1), /\bcursor-pointer\b/)
  assert.doesNotMatch(requiredAt(titleVariantButtonMatch, 1), /\bbg-neutral-500\/4\.5/)
  assert.match(requiredAt(titleVariantButtonMatch, 1), /\[--capsule-fill:transparent\]/)
  assert.match(requiredAt(titleVariantButtonMatch, 1), /rounded-full/)
  ...
  assert.match(requiredAt(titleVariantButtonMatch, 1), /hover:\[--capsule-fill:var\(--chip-target-interaction-bg\)\]/)
  assert.doesNotMatch(requiredAt(titleVariantButtonMatch, 1), /\bcursor-default\b/)
  const pageChipSource = readFileSync(new URL('../src/components/PageChip.tsx', import.meta.url), 'utf8')
  ...
  assert.match(pageChipSource, /variantHoverMatched && '\[--capsule-fill:var\(--chip-target-interaction-bg\)\] text-tab-live'/)
  assert.match(pageChipSource, /row\.current && '\[--capsule-fill:color-mix\(in_oklab,var\(--color-neutral-600\)_10%,transparent\)\] text-tab-live'/)
  assert.match(pageChipSource, /row\.active && '\[--capsule-fill:color-mix\(in_oklab,var\(--color-neutral-600\)_7\.5%,transparent\)\] text-tab-live'/)
...
test('PageChip gives a plain open chip the shared closed-page interaction outline', () => {
  ...
  assert.match(cls, /data-\[tabout-filter-result-selected=true\]:\[--capsule-fill:var\(--chip-interaction-bg\)\]/)
...
test('PageChip gives read-only filter results the closed interaction fill and outline', () => {
  for (const sourceType of ['bookmark', 'history'] as const) {
    ...
    const cls = pageChipClass(html)
    assert.match(cls, /hover:\[--capsule-fill:var\(--chip-interaction-bg\)\]/)
    assert.match(cls, /data-\[tabout-filter-result-selected=true\]:\[--capsule-fill:var\(--chip-interaction-bg\)\]/)
    assert.match(cls, /data-\[tabout-filter-result-selected=true\]:outline-\(--accent-amber\)/)
    assert.match(html, /--chip-interaction-bg:color-mix\(in srgb, var\(--card-bg\) 96\.5%, var\(--color-neutral-600\) 3\.5%\)/)
    assert.match(html, /--chip-hover-border:color-mix\(in srgb, var\(--color-neutral-600\) 22%, transparent\)/)
...
test('PageChip expands same-title URL variant groups in place', () => {
  ...
  const layoutPolicySource = readFileSync(new URL('../src/components/page-chip-text-layout/policy.ts', import.meta.url), 'utf8')
  assert.match(layoutPolicySource, /Math\.max\(rect\.width, minWidth, contentMetrics\.width \+ horizontalInset\)/)
  assert.doesNotMatch(layoutPolicySource, /Math\.max\(minWidth, contentMetrics\.width \+ horizontalInset\)/)
  const layoutFragmentsSource = readFileSync(new URL('../src/components/page-chip-text-layout/fragments.ts', import.meta.url), 'utf8')
  assert.match(layoutFragmentsSource, /chip-title-variant-list inline-flex max-w-full flex-col items-stretch gap-px align-top'/)
  assert.match(layoutFragmentsSource, /chip-title-variant inline-flex max-w-full min-w-0 items-center gap-1 rounded-full bg-transparent px-1\.5 py-\[2\.5px\]/)
...
test('PageChip keeps icon-only tooltip popups click-through while text chips expand in place', () => {
  ...
  assert.match(pageChipSource, /page-chip-tooltip max-w-\[calc\(100vw-16px\)\] text-\[13px\] leading-tight wrap-break-word select-none', parentInteractive \? chipCursorClass : 'cursor-default'/)
})
...
test('PageChip routes saved-page mutation actions through Base UI context menus', () => {
  ...
  const historyEntryRowSource = readFileSync(new URL('../src/components/history-entry/HistoryEntry.tsx', import.meta.url), 'utf8')
  assert.match(historyEntryRowSource, /data-tabout-part="focus-button"[\s\S]*aria-label=\{entryLabel\}/)
  ...
  assert.match(historyEntryRowSource, /onReloadSelect=\{canShowSuspend \? onReloadEntry : undefined\}/)
  assert.match(historyEntryRowSource, /onDuplicateSelect=\{canShowSuspend \? onDuplicateEntry : undefined\}/)
})
...
test('TabHistoryPanel renders the close action in the favicon slot', () => {
  ...
  const historyRowCloseSource = readFileSync(new URL('../src/components/history-entry/HistoryEntry.tsx', import.meta.url), 'utf8')
  assert.doesNotMatch(historyRowCloseSource, /<TooltipAnchor content="Close this tab">/)
})
...
test('TabHistoryPanel uses PageChip-style fade truncation and in-place title expansion', () => {
  ...
  const historyEntryComponentSource = readFileSync(new URL('../src/components/history-entry/HistoryEntry.tsx', import.meta.url), 'utf8')
  const historyEntryExpansionSource = readFileSync(new URL('../src/components/history-entry/use-history-entry-expansion.ts', import.meta.url), 'utf8')
  const historyTitleMeasurementSource = readFileSync(new URL('../src/components/history-entry/title-measurement.ts', import.meta.url), 'utf8')
  const historySurfacesSource = [tabHistoryPanelSource, historyEntryComponentSource, historyEntryExpansionSource, historyTitleMeasurementSource].join('\n')
  assert.match(historyTitleMeasurementSource, /titleEl\.scrollHeight - titleEl\.clientHeight > 1/)
  assert.match(historyTitleMeasurementSource, /getHistoryTitleContentWidth/)
  assert.match(historyTitleMeasurementSource, /getHistoryTitleExpandedLineHtml/)
  assert.match(historyTitleMeasurementSource, /historyTitleExpandedLineMarkup/)
  assert.match(historyEntryComponentSource, /expansionLineNodesFromHtml/)
  assert.match(historyTitleMeasurementSource, /expandedViewportConstrained/)
  assert.match(historyTitleMeasurementSource, /visibleLineCount/)
  assert.match(historyEntryComponentSource, /history-entry-title block min-w-0 flex-auto overflow-hidden hyphens-auto break-normal max-h-\[calc\(2lh\)\]/)
  assert.match(historyEntryComponentSource, /\[\&\.history-entry-title-truncated\]:mask-\(--title-fade-mask\)/)
  assert.doesNotMatch(historyEntryComponentSource, /history-entry-title line-clamp-2/)
  assert.match(
    historyTitleMeasurementSource,
    /captureVisibleLineHtml\(titleEl, metrics\.visibleLineCount, captureGeometry\)/,
  )
  assert.match(historyEntryComponentSource, /clampedTitleLineNodes\(clampedLineHtml, 'history-entry-title'\)/)
  assert.match(historyEntryComponentSource, /key=\{titleContentKey\}[\s\S]*className="captured-title-content-root contents"/)
  const pageChipClampSource = readFileSync(new URL('../src/components/PageChip.tsx', import.meta.url), 'utf8')
  const chipLayoutDomSource = readFileSync(new URL('../src/components/page-chip-text-layout/dom.ts', import.meta.url), 'utf8')
  assert.match(
    chipLayoutDomSource,
  ...
  assert.match(pageChipClampSource, /clampedLines\([^?\n]+\? rebuildClampedChipMarker : undefined\)/)
  ...
  assert.match(historyTitleMeasurementSource, /history-entry-title-expansion-measure/)
  assert.match(historyEntryComponentSource, /--history-entry-expanded-width/)
  assert.match(historyEntryComponentSource, /w-\(--history-entry-expanded-title-width\)/)
  assert.match(historyEntryExpansionSource, /const \[titleExpanded, setTitleExpandedState\] = useState\(false\)/)
  ...
  assert.match(historyEntryComponentSource, /HISTORY_ENTRY_INTERACTION_CLASSES = 'title-interaction:hover:\[--capsule-fill:var\(--history-entry-interaction-bg\)\][\s\S]*\[\&\.history-entry-expanded-open\]:\[--capsule-fill:var\(--history-entry-interaction-bg\)\]/)
  assert.match(historyEntryComponentSource, /HISTORY_ENTRY_ACTIVE_OTHER_INTERACTION_CLASSES = `\[--capsule-fill:var\(--history-entry-rest-bg\)\]/)
  assert.match(historyTitleMeasurementSource, /history-entry-expanded-line-tail/)
  assert.match(historyTitleMeasurementSource, /function getHistoryEntryExpansionGeometry\(entryEl: HTMLElement \| null, titleEl: HTMLElement \| null\)/)
  assert.match(historyEntryComponentSource, /const expandedEntryElement = titleExpanded \? historyEntrySurface\(true\) : null/)
  ...
  assert.doesNotMatch(historySurfacesSource, /onScrollCapture=/)
  assert.doesNotMatch(historySurfacesSource, /window\.addEventListener\('scroll'/)
  assert.doesNotMatch(historySurfacesSource, /function getHistoryEntryLayerRoot/)
  assert.doesNotMatch(historySurfacesSource, /layerElement\?: HTMLElement \| null/)
  assert.doesNotMatch(historySurfacesSource, /const \[historyPanelElement, setHistoryPanelElement\]/)
  assert.doesNotMatch(historySurfacesSource, /createPortal/)
  assert.doesNotMatch(historySurfacesSource, /history-entry-overlay/)
  assert.doesNotMatch(historySurfacesSource, /onHistoryEntryWheel/)
  ...
  assert.doesNotMatch(historySurfacesSource, /w-max/)
  assert.doesNotMatch(historySurfacesSource, /TooltipAnchor/)
  assert.doesNotMatch(historySurfacesSource, /getHistoryTitleTooltipAnchor/)
  assert.doesNotMatch(historySurfacesSource, /history-entry-title-tooltip/)
  assert.match(historyEntryComponentSource, /after:w-0/)
  assert.doesNotMatch(historyEntryComponentSource, /after:w-14/)
})
...
test('TabHistoryPanel borrows current PageChip surface styling for the current entry', () => {
  ...
  assert.match(defaultEntry, /\[--capsule-fill:transparent\]/)
  assert.match(defaultEntry, /rounded-page-chip/)
  assert.match(defaultEntry, /hover:\[--capsule-fill:var\(--history-entry-interaction-bg\)\]/)
  ...
  assert.match(currentEntry, /\[--capsule-fill:var\(--color-neutral-100\)\]/)
  assert.match(currentEntry, /\bring-neutral-400\b/)
  assert.doesNotMatch(currentEntry, /hover:border-\(--accent-amber\)/)
  assert.doesNotMatch(currentEntry, /\bhover:\[--capsule-fill:var\(--history-entry-interaction-bg\)\]\b/)
  ...
  assert.match(entries.find((entry) => entry.tag.includes('data-current="true"'))?.tag ?? '', /--history-entry-fade-bg:var\(--color-neutral-100\)/)
...
test('TabHistoryPanel borrows other-window PageChip surface styling for active non-current entries', () => {
  ...
  assert.match(activeOtherEntry, /\[--capsule-fill:var\(--history-entry-rest-bg\)\]/)
  assert.match(activeOtherEntry, /shadow-\[0_1px_2px_rgba\(10,10,10,0\.04\)\]/)
  assert.match(activeOtherEntry, /hover:\[--capsule-fill:var\(--history-entry-interaction-bg\)\]/)
...
test('PageChip renders a title suppression marker when common title text is suppressed', () => {
  ...
  assert.match(requiredAt(markerMatch, 1), /(?:^|\s)leading-3\.5(?:\s|$)/)
  ...
  const pageChipSource = readFileSync(new URL('../src/components/PageChip.tsx', import.meta.url), 'utf8')
  assert.match(pageChipSource, /const shouldExpandChip = !chip\.iconOnly && \(hasExpandableContent \|\| chipTextLayout\.hasClampedLines \|\| hasTitleSuppressionMarkers \|\| hasStructuralPlaceholders\)/)
  const layoutFragmentsSource = readFileSync(new URL('../src/components/page-chip-text-layout/fragments.ts', import.meta.url), 'utf8')
  assert.match(layoutFragmentsSource, /PAGE_CHIP_TOOLTIP_SUPPRESSION_MARKER_CLASS_NAME = 'chip-title-suppression-marker inline-block border-0[\s\S]*text-\[12px\][\s\S]*leading-3\.5[\s\S]*align-middle/)
  assert.match(layoutFragmentsSource, /PAGE_CHIP_TOOLTIP_SUPPRESSION_MARKER_CLASS_NAME = [\s\S]*\[box-decoration-break:clone\]/)
...
test('PageChip expansion preserves trailing suppression marker spacing', () => {
  ...
  assert.match(layoutPolicySource, /function carriedExpandedMarkerSpacingClass\(classNames: readonly string\[\]\)/)
  assert.match(layoutPolicySource, /className\.startsWith\('ml-'\) \|\| className\.startsWith\('mr-'\)/)
  assert.match(layoutFragmentsSource, /if \(carriedExpandedMarkerSpacingClass\(Array\.from\(marker\.classList\)\)\) return/)
  assert.match(layoutFragmentsSource, /PAGE_CHIP_TOOLTIP_SUPPRESSION_MARKER_CLASS_NAME, carriedExpandedMarkerSpacingClass\(markerClasses\), carriedExpandedMarkerToneClass\(markerClasses\)/)
  assert.match(pageChipSource, /chip-title-suppression-label hidden group-\[\.page-chip-expanded\]\/page-chip:inline/)
...
test('PageChip labels stripped path-group placeholders with the pathgroup value', () => {
  ...
  const layoutFragmentsSource = readFileSync(new URL('../src/components/page-chip-text-layout/fragments.ts', import.meta.url), 'utf8')
  assert.match(layoutFragmentsSource, /chip-strip-indicator inline-block max-w-full/)
...
test('PageChip marks chips affected by the active suppressed title text', () => {
  ...
  assert.match(defaultHtml, /\[--capsule-fill:var\(--color-yellow-50\)\]/)
  ...
  assert.match(tealHtml, /\[--capsule-fill:var\(--color-teal-50\)\]/)
  assert.match(tealHtml, /chip-title-suppression-marker\b[^"]*\[--capsule-fill:var\(--color-teal-50\)\]/)
  assert.doesNotMatch(tealHtml, /\[--capsule-fill:var\(--color-yellow-50\)\]/)
})
...
test('PageChip renders folded titles before env controls', () => {
  ...
  assert.doesNotMatch(requiredAt(chipMatch, 1), /hover:border/)
  assert.doesNotMatch(requiredAt(chipMatch, 1), /hover:ring/)
  assert.doesNotMatch(requiredAt(chipMatch, 1), /(?:^|\s)focus-visible:outline/)
  ...
  assert.match(requiredAt(envButtonMatch, 1), /\brounded-md\b/)
  assert.match(requiredAt(envButtonMatch, 1), /\bclickable\b/)
  assert.match(requiredAt(envButtonMatch, 1), /\bcursor-pointer\b/)
  assert.doesNotMatch(requiredAt(envButtonMatch, 1), /\bcursor-default\b/)
...
test('closed-ghost rows declare a forget affordance instead of a tab-close', () => {
  ...
  const source = readFileSync(new URL('../src/components/history-entry/HistoryEntry.tsx', import.meta.url), 'utf8')
